 ...
 * 
 * REFACTORISÉ le 2026-02-14
 * - Les parsers internes (_BM_*, __*) ont été centralisés dans BM_Parsers.gs
 * - Tous les appels utilisent maintenant BM_PARSERS_*
 * - Fonctionnalités identiques, structure améliorée
 * 
 * ⚠️ DÉPENDANCE: Requiert BM_Parsers.gs dans le même projet
 * ⚠️ PROTECTION: Ne modifie PAS R06_IA_MEMORY, VALIDATION_GATE, OCR pipeline
 */
...
function traiterNouveauDocument(fichier) {
  try {
    ...
    // IMPORTANT: ocrText / ocrEngine doivent exister AVANT OCR1_ENRICH
      ...
      const _best = BM_PARSERS_pickLongestText([_cand1, _cand2, _cand3]);
...
function __setIfMissing__(obj, key, val, opts) {
  const o = opts || {};
  if (!obj) return;
  if (!BM_PARSERS_isEmpty(obj[key])) return;
  if (BM_PARSERS_isEmpty(val)) return;
...
__setIfMissing__(donnees, "client", String(f.client_nom || f.client || ""), { rejectUnknown: true });
        ...
        if (!donnees.date_document) {
          const __d = BM_PARSERS_normDateSwapYMD(__dateSrc);
        ...
        if (__typeUpper === "BON_LIVRAISON" || __typeUpper === "BON LIVRAISON" || __typeUpper === "BL") {
          ...
          const __email = BM_PARSERS_extractEmail(donnees.client_nom || donnees.client || "");
          const __socUp = String(donnees.societe || "").toUpperCase();
          if (__email && __socUp.indexOf("MARTIN") >= 0) {
            const __supplier = BM_PARSERS_supplierNameFromEmail(__email);
            if (__supplier) {
...
// Fallback OCR (si CloudRun n'a pas renvoyé cp/ville)
      ...
      if (!donnees.numero_facture) {
        const _num = BM_PARSERS_extractInvoiceNumber(ocrText);
        if (_num) donnees.numero_facture = _num;
      }
      if (!donnees.ttc || !donnees.ht || !donnees.tva_montant) {
        const _amts = BM_PARSERS_extractAmounts(ocrText);
